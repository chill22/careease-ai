...
export default function Intake() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    setIsLoading(true);
    // We'll add the Groq call in the next file
    ...
    <div className="min-h-screen py-12 px-4">
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-indigo-900">Let’s create your free care plan</h1>
          <p className="mt-4 text-xl text-indigo-700">Takes less than 60 seconds • 100% private • Made just for you</p>
        </div>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-indigo-100">
              <h2 className="text-2xl font-semibold text-indigo-900 mb-6">About your loved one</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormField control={form.control} name="lovedOneName" render={({ field }) => (
                  <FormItem>
                    <FormLabel>First name (or nickname)</FormLabel>
                    <FormControl><Input placeholder="e.g. Mom, Dad, Maria" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <FormField control={form.control} name="age" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input type="number" placeholder="78" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-teal-100">
              <h2 className="text-2xl font-semibold text-teal-900 mb-6">Health & daily needs (check all that apply)</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormField control={form.control} name="conditions" render={() => (
                  <FormItem>
                    <FormLabel>Common conditions</FormLabel>
                    {["Dementia / Alzheimer’s", "Parkinson’s", "Stroke recovery", "Diabetes", "Heart disease", "Cancer", "Arthritis", "None of these"].map((item) => (
                      <FormField key={item} control={form.control} name="conditions" render={({ field }) => {
                        return (
                ...
                <div className="space-y-6">
                  <FormField control={form.control} name="mobility" render={({ field }) => (
                    <FormItem>
                      ...
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        <FormControl><SelectTrigger><SelectValue placeholder="How do they get around?" /></SelectTrigger></FormControl>
                        <SelectContent>
                  ...
                  <div className="space-y-4">
                    <FormField control={form.control} name="memoryIssues" render={({ field }) => (
                      <FormItem className="flex items-center space-x-3">
                        <FormControl><Checkbox checked={field.value} onCheckedChange={field.onChange} /></FormControl>
                        <FormLabel className="font-normal">Has memory issues or confusion</FormLabel>
                      </FormItem>
                    ))} />
                    <FormField control={form.control} name="needsHelpEating" render={({ field }) => (
                      <FormItem className="flex items-center space-x-3">
                        <FormControl><Checkbox checked={field.value} onCheckedChange={field.onChange} /></FormControl>
                        <FormLabel className="font-normal">Needs help eating or preparing meals</FormLabel>
                      </FormItem>
                    ))} />
                    <FormField control={form.control} name="needsHelpBathing" render={({ field }) => (
                      <FormItem className="flex items-center space-x-3">
                        <FormControl><Checkbox checked={field.value} onCheckedChange={field.onChange} /></FormControl>
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-emerald-100">
              <FormField control={form.control} name="mainChallenges" render={({ field }) => (
                <FormItem>
                  <FormLabel>What’s the hardest part of caregiving right now?</FormLabel>
                  <FormControl>
                    <Textarea placeholder="e.g. I’m exhausted by 6pm, Mom wanders at night, I can’t find respite care…" className="min-h-32" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )} />
            ...
            <div className="text-center">
              <Button type="submit" size="lg" disabled={isLoading} className="text-lg px-12 py-7 bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-lg">
                {isLoading ? <Loader2 className="mr-2 h-6 w-6 animate-spin" /> : <ArrowRight className="mr-2 h-6 w-6" />}
                {isLoading ? "Creating your plan…" : "Show Me My Free Care Plan"}
              </Button>
              <p className="mt-4 text-sm text-gray-600">Takes about 15 seconds • Completely private</p>
            </div>
          </form>
        </Form>
      </div>
